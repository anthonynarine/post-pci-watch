...
export function SyntheticDataNotice({ snapshotAt }: { snapshotAt: string }) {
  return (
    <div className="flex flex-wrap items-start gap-3 rounded-xl border border-primary/30 bg-primary-soft px-4 py-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 text-xs text-foreground-muted">
          Every patient detail, measurement, and event on this page is fabricated for
          development. This is not a medical device, it is not connected to any wearable, and
          it must not be used for clinical decision-making.
        </p>
      </div>
      <p className="text-xs text-foreground-muted">
        Snapshot <span className="font-mono">{snapshotAt}</span>
      </p>
    </div>
  );
}
